import"./modulepreload-polyfill.b7f2da20.js";import{T as O,S,P as _,W as E,G,b as L,f as V,g as W,u as b,o as H,E as A,n as c,v as j,w as Q,x as Y,A as X,d as Z,y as I,i as J}from"./vendor.7fb6b1a4.js";const q=`
varying vec2 vUv;
uniform float iTime;
...
`;var N="/playground1/assets/tex1.58cc41e0.png",$="/playground1/assets/b2.0dc69ad7.png",ee="data:image/png;base64,";const y=new O,oe=y.load(N),ae=y.load($),ie=y.load(ee),C={width:window.innerWidth,height:window.innerHeight},te=window.innerWidth/window.innerHeight,g=document.getElementById("canvas"),x=new S,v=new _(90,C.width/C.height,1,1e3);v.position.set(0,0,50);const w=new E({canvas:g,antialias:!1,alpha:!1});w.setSize(window.innerWidth,window.innerHeight);w.setPixelRatio(Math.min(window.devicePixelRatio,1));const e={iTime:{type:"f",value:100},iResolution:{type:"v2",value:new c},iMouse:{type:"v2",value:new c},audio1:{type:"f",value:1},adj:{type:"f",value:0},orbOpacity:{type:"f",value:1},intensity:{type:"f",value:1},iChannel0:{type:"t",value:oe},surfaceDensity:{type:"f",value:1.2},mouseReal:{type:"v2",value:new c},mouseDown:{type:"v2",value:new c}};e.iResolution.value.x=window.innerWidth;e.iResolution.value.y=window.innerHeight;e.adj.value=.2-window.innerHeight/window.innerWidth;new G;const h=[];function ne(){for(var o=new j,t=[],r=[],a,i=0;i<350;i++){var D=Math.random()*60-30,B=Math.random()*60-30,k=Math.random()*60-30;t.push(D,B,k)}o.setAttribute("position",new Q(t,3)),a=[[[.3,.7,.9],ae,.2],[[.3,.3,.8],ie,.4]];for(var i=0;i<a.length;i++){var f=a[i][0],U=a[i][1],P=a[i][2];r[i]=new Y({size:P,map:U,blending:X,depthTest:!1,transparent:!0,opacity:.35}),r[i].color.setRGB(f[0],f[1],f[2]);var d=new Z(o,r[i]);d.rotation.x=Math.random()*6,d.rotation.y=Math.random()*6,d.rotation.z=Math.random()*6,h.push(d),x.add(d)}}let re=new L({uniforms:e,vertexShader:q,fragmentShader:K});var le=new V(1,1);const M=new W(le,re);M.scale.set(110*te,110,1);x.add(M);ne();var R=new J;let se=1;function F(){requestAnimationFrame(F);const o=R.getElapsedTime();if(R.getDelta()*100,e.iTime.value+=.02,e.audio1.value=128/48+Math.random()*.1,e.iMouse.value.x=e.iMouse.value.x+(m.x-e.iMouse.value.x)*.05,e.iMouse.value.y=e.iMouse.value.y+(m.y-e.iMouse.value.y)*.05,e.mouseReal.value.x=p.x,e.mouseReal.value.y=p.y,e.mouseDown.value.x=n.x,e.mouseDown.value.y=n.y,e.intensity.value=se,!(l>0||(s==null?void 0:s.isPlaying())||(u==null?void 0:u.isPlaying()))){let a=e.surfaceDensity.value+Math.sin(o*.5)*.01;e.surfaceDensity.value=Math.min(2,Math.max(.7,a))}for(var t=0;t<h.length;t++){var r=h[t];let a=t<1?1:-2;r.rotation.x=.01*e.iTime.value*a,r.rotation.z=-.03*e.iTime.value*a}v.position.x+=(-m.x*.01-v.position.x)*.05,v.position.y+=(m.y*.01-v.position.y)*.05,I(),w.render(x,v)}let m={x:1,y:1},p={x:1,y:1},l=0,T=new c(0,0),z=new c(0,0);function ue(o){var t=o.clientX-window.innerWidth/2,r=o.clientY-window.innerHeight/2;if(m.x=t,m.y=r,p.x=o.clientX,p.y=window.innerHeight-o.clientY,z.set(p.x,o.clientY),n.y>0){(n.y-o.clientY)/window.innerHeight,l=T.distanceTo(z)/window.innerHeight;const a=1+-l*.9;e.surfaceDensity.value=Math.max(.3,a),console.log(e.surfaceDensity.value)}}let s;document.addEventListener("mousemove",ue,{passive:!0});g.addEventListener("click",()=>{console.log("fake click",l),l<.001&&(console.log("real click",l),s&&s.stop(),s=new b(e.orbOpacity).to({value:1+H.randFloat(-.7,1)},1e3).easing(A.Quadratic.Out).start())});let n={x:0,y:0},u;g.addEventListener("mousedown",o=>{n.y=o.clientY,n.x=o.clientX,T.set(n.x,n.y),u&&u.stop(),s&&s.stop()},{passive:!0});g.addEventListener("mouseup",o=>{n.y=0,n.x=0,u&&u.stop(),l>0&&(u=new b(e.surfaceDensity).to({value:1.1},700).easing(A.Quadratic.Out).onComplete(t=>{l=0}).start())});F();
//# sourceMappingURL=morb.23218f06.js.map
